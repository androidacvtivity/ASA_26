...
// ============================================
// ADDON ASA23: CAP4_R_C31 -> auto set CAP4_R_C32 (CAEM2)
// folosim set_caem_to_select(selector, valueCaem, keyCaem)
// ============================================
(function ($) {
    ...
    function getKeyFromId(id, base) {
        ...
        return m ? parseInt(m[1], 10) : null; // keyCaem e 1-based (cum folosește și set_options_html)
    }
    ...
    function cleanCode(v) {
        return $.trim(String(v || "")).replace(/[^\d]/g, "");
    }
    ...
    function syncC31toC32($inp) {
        ...
        var code = cleanCode($inp.val());

        asa23_lock = true;
        try {
            if (code.length === 4) {
                // umple select-ul și selectează codul (dacă există)
                set_caem_to_select("CAP4_R_C32", code, key);

                // dacă codul nu există în opțiuni, rămâne gol
                var $sel = getObj("CAP4_R_C32", key);
                if ($sel.length && $sel.val() !== code) {
                    $sel.val("").change();
                }
            } else {
                // dacă nu e cod valid, golim select-ul din rând
                var $sel2 = getObj("CAP4_R_C32", key);
                if ($sel2.length) $sel2.val("").change();
            }
        } finally {
            asa23_lock = false;
        }
    ...
    function bindHandlers() {
        ...
        // evităm dublări la attach/afterLoad
        $form.off(".asa23_caem_addon");

        // C31 -> C32
        $form.on("input.asa23_caem_addon change.asa23_caem_addon",
            "input[id^='CAP4_R_C31']",
    ...
    // IMPORTANT: fără să modificăm funcția ta existentă afterLoad. O „împachetăm”.
